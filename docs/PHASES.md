...
Detailed build log for [dy-dashboard](../README.md) — what was done, why, and every bug found and fixed along the way. See the [README](../README.md) for the quick overview and live demo link.
...
- [x] Branch protection enabled — required PR before merge, `enforce_admins: true` (applies even to the repo owner), force-push and branch deletion blocked. Verified: a direct `git push origin main` was rejected. Required status checks (green CI) were added once the workflow existed in Phase 6.
...
- [x] Preview deploy verified on a PR — [PR #3](https://github.com/nathmsi/dy-dashboard/pull/3) got its own disposable preview URL automatically; used it to confirm the SPA rewrite fix (below) actually works before merging to prod
